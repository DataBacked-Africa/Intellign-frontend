...
const AppSidebar: React.FC<AppSidebarProps> = ({ isOpen, setIsOpen }) => {
    ...
    }, [sessionId, removeSession, clearSession, router]);
        ...
        >
            ...
            <div className="p-3 pt-4 flex items-center justify-between">
                ...
                {isOpen && (
                    <button
                        ...
                        className="p-1.5 rounded-md hover:bg-[var(--brand-bone-deep)] text-[var(--fg-muted)] transition-colors"
                        title="Close sidebar"
                    >
            ...
            <div className="px-3 pb-2 space-y-0.5">
                <Link
                    ...
                    className="flex items-center gap-3 w-full p-2.5 rounded-lg hover:bg-[var(--brand-bone-deep)] transition-colors text-sm text-[var(--fg-primary)]"
                >
                    <SquarePen className="w-4 h-4 text-[var(--fg-tertiary)]" />
                    {isOpen && <span>New chat</span>}
                </Link>

                <div className="flex items-center gap-3 w-full p-2.5 rounded-lg bg-[var(--brand-bone-deep)] hover:bg-[var(--neutral-200)] focus-within:ring-2 focus-within:ring-[var(--brand-maroon)] transition-colors text-sm text-[var(--fg-primary)]">
                    <Search className="w-4 h-4 text-[var(--fg-tertiary)] shrink-0" />
                    {isOpen && (
                        <input
                            type="text"
                            placeholder="Search chats"
                            className="flex-1 bg-transparent outline-none text-sm text-[var(--fg-primary)] placeholder:text-[var(--fg-muted)] min-w-0"
                            value={searchQuery}
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar py-2 px-3">
                {sessions.length > 0 && isOpen && (
                    <p className="mb-2 px-2 text-xs font-semibold text-[var(--fg-primary)] mt-2">Recents</p>
                )}

                <div className="space-y-0.5">
                    {filteredSessions.length === 0 && searchQuery && isOpen ? (
                        <p className="px-2 text-sm text-[var(--fg-muted)] py-4 text-center">No chats found.</p>
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    isActive ? 'bg-[var(--brand-bone-deep)]' : 'hover:bg-[var(--brand-bone-deep)]',
                                    ...
                                    {isOpen && (
                                        <span className="truncate text-sm text-[var(--fg-secondary)]">
                                            {isGenerating && !session.name ? (
                                                ...
                                                <span className="flex items-center gap-1.5 text-[var(--neutral-400)]">
                                                    <Loader2 className="w-3 h-3 animate-spin" />
                                ...
                                {isOpen && !isDeleting && (
                                    <div className="flex items-center gap-0.5 pr-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            ...
                                            className="p-1.5 rounded-md hover:bg-[var(--neutral-200)] text-[var(--neutral-400)] hover:text-[var(--fg-tertiary)] transition-colors"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="p-1.5 rounded-md hover:bg-red-900/20 text-[var(--neutral-400)] hover:text-red-500 transition-colors"
                                        >
            ...
            {/* User profile */}
            <div className="p-3">
                <div className="flex items-center gap-3 w-full p-2 rounded-lg hover:bg-[var(--brand-bone-deep)] transition-colors text-[var(--fg-secondary)]">
                    <div className="w-8 h-8 rounded-full bg-[var(--brand-maroon)]/10 flex items-center justify-center text-[var(--brand-maroon)] font-medium shrink-0">
                        {user?.name?.charAt(0)?.toUpperCase() ?? 'U'}
                    </div>
                    {isOpen && (
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-[var(--fg-primary)] truncate">{user?.name ?? 'User'}</p>
