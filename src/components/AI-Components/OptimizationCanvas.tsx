...
const ResultsTab: React.FC<{ jobId: string }> = ({ jobId }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Approval status */}
            <div className="rounded-xl p-4" style={{ background: '#fff', border: '1px solid var(--border-subtle)' }}>
            ...
            {/* Assignments table */}
            <div className="rounded-xl overflow-hidden" style={{ background: '#fff', border: '1px solid var(--border-subtle)' }}>
...
const LOGIC_LABELS: Record<string, string> = { weighted_scoring: 'Weighted Scoring', categorical_match: 'Attribute Matching', spatial_proximity: 'Distance / Proximity', numeric_threshold: 'Numeric Threshold', set_coverage: 'Skill Coverage', temporal_match: 'Schedule Matching' };

interface GoalsTabProps { goals: GoalDefinition[]; sessionId: string | null }

const GoalsTab: React.FC<GoalsTabProps> = ({ goals, sessionId }) => {
    ...
    const persistGoals = async (updated: GoalDefinition[]) => {
        if (!sessionId) return;
        setSaving(true);
        try {
            // Backend POST /config/goals/{session_id} expects List[GoalDefinition] directly
            await axiosInstance.post(`/config/goals/${sessionId}`, updated);
            showToast.success('Goals saved', '');
        } catch { showToast.error('Save failed', 'Could not persist goals.'); }
    ...
    const inputStyle = { border: '1px solid var(--border-subtle)', background: '#fff', color: 'var(--fg-primary)', fontFamily: 'inherit' };
    ...
    return (
        <div className="space-y-3">
            ...
            {localGoals.map((g, idx) => {
                ...
                return (
                    <div key={idx} className="rounded-xl overflow-hidden" style={{ background: '#fff', border: `1px solid ${isEditing ? 'var(--brand-maroon)' : 'var(--border-subtle)'}`, transition: 'border-color 140ms' }}>
                        <div className="flex items-center gap-3 p-4">
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded uppercase shrink-0"
                                style={{ background: g.award_type === 'Reward' ? '#D1FAE5' : '#FEE2E2', color: g.award_type === 'Reward' ? '#047857' : '#B91C1C' }}>
                                {g.award_type}
                            </span>
            ...
            {localGoals.length === 0 && !addingNew && (
                <div className="py-10 text-center">
                    ...
                    <p className="text-sm" style={{ color: 'var(--fg-tertiary)' }}>Click "Add goal" or describe goals in the chat.</p>
                </div>
            )}
        </div>
    );
...
const DatasetsTab: React.FC<{ sessionId: string | null; dataContext: DataContext | null }> = ({ sessionId, dataContext }) => {
    ...
    return (
        <div className="space-y-5">
            {tables.map(t => (
                <div key={t.key} className="rounded-xl overflow-hidden" style={{ background: '#fff', border: '1px solid var(--border-subtle)' }}>
                    {/* Header */}
...
const OptimizationCanvas: React.FC = () => {
    ...
    return (
        <aside style={{ width: 720, flexShrink: 0, background: '#fff', borderLeft: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', animation: 'canvas-in 280ms cubic-bezier(0.2,0,0,1)' }}>
            {/* Head */}
