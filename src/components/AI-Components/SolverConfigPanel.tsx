          ...
          return (
            <button
              ...
              className={cn(
                ...
                active
                  ? "border-[#6B1D1D] bg-[#6B1D1D]/8 text-[#6B1D1D]"
                  : "border-gray-200 text-gray-500 hover:border-[#6B1D1D]/40",
              )}
            >
              {m.icon}
      ...
      <div className="flex items-center justify-between">
        ...
        {!editing
          ...
          : <div className="flex gap-1">
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs bg-[#6B1D1D] text-white hover:bg-[#6B1D1D]/90 disabled:opacity-50"><Check className="w-3 h-3" /> {saving ? "Saving…" : "Save"}</button>
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden">
        {Object.entries(editing ? draft : config.params).map(([key, value], i, arr) => (
          <div key={key} className={cn("flex items-center justify-between px-3 py-2.5", i < arr.length - 1 && "border-b border-gray-100", i % 2 === 0 ? "bg-white" : "bg-gray-50/50")}>
            ...
            {editing
              ? <input type="text" value={String(draft[key] ?? "")} onChange={e => setDraft(d => ({ ...d, [key]: e.target.value }))} className="w-24 px-2 py-1 text-xs border border-gray-200 rounded-lg text-right focus:outline-none focus:border-[#6B1D1D]/50" />
