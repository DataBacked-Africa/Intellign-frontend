  ...
  return (
    <div className="flex flex-col gap-4 p-4 h-full justify-center">
      <div className="flex items-center gap-3 mb-2">
        <Loader2 className="w-5 h-5 text-[#6B1D1D] animate-spin flex-shrink-0" />
        <div>
      ...
      <div>
        ...
        <div className="w-full bg-gray-100 rounded-full h-2">
          <div
            className="h-2 rounded-full bg-[#6B1D1D] transition-all duration-500"
      ...
      <table className="w-full border-collapse">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            {preview.columns.length > 8 && <th className="px-3 py-2 text-gray-400">+{preview.columns.length - 8}</th>}
          </tr>
        </thead>
        <tbody>
          {preview.rows.slice(0, 10).map((row, ri) => (
            <tr key={ri} className="border-t border-gray-100 even:bg-gray-50/40">
              ...
              {preview.columns.length > 8 && <td className="px-3 py-2 text-gray-400">…</td>}
            </tr>
          ))}
        </tbody>
      </table>
...
export const DatasetPanel: React.FC<DatasetPanelProps> = ({
  ...
  const subscribeSSE = useCallback(() => {
    ...
    es.onmessage = (ev) => {
      try {
        const data: GenerationStatus = JSON.parse(ev.data);
        setGenStatus(data);
